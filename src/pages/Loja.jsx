...
export default function Home() {
  ...
  return (
    <div className="bg-white min-h-screen w-full"> {/* Aplicar fundo branco e garantir que ele ocupe toda a tela */}
      {/* Alinhando corretamente o saldo */}
      <div className="max-w-screen-xl mx-auto p-4"> {/* Limitar o conteúdo centralizado */}
        <div className="flex flex-wrap justify-end items-center mb-4 space-x-2 sm:space-x-4">
          <div className="bg-[#C0C0C0] border border-gray-600 rounded-full text-center px-4 py-2 flex items-center">
            <img src={moedaIcon} className="w-8 h-8 mr-2" alt="Turbo Coins" />
            <p className="text-black font-semibold">TC {saldo}</p>
          </div>
          ...
          <input
            ...
            className="bg-[#FFFFFF] border border-gray-600 text-black p-2 rounded-full w-28 sm:w-40 flex-shrink-0"
          />
          
          <button
            onClick={aplicarCodigoPromocional}
            className="bg-[#0000FF] border border-gray-600 text-white p-2 rounded-full w-20 sm:w-24 flex-shrink-0 hover:bg-blue-600 transition"
          ...
          <button
            onClick={toggleModal}
            className="bg-[#0000FF] border border-gray-600 text-white p-2 rounded-full w-10 flex-shrink-0 hover:bg-blue-600 transition"
          >
            i
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mt-6">
          {produtos.map((produto) => (
            <div
              key={produto.id}
              className="bg-[#C0C0C0] border border-blue-600 hover:border-blue-700 rounded-lg p-6 text-center shadow-lg flex flex-col justify-between transition"
            >
              {produto.img ? (
                <img
                  src={produto.img}
              ...
              <div className="flex-grow">
                <h3 className="text-black font-bold text-lg mb-2">{produto.nome}</h3>
                <p className="text-gray-800 text-sm mb-4">TC {produto.preco.toFixed(2)}</p>
              </div>
              <button
                onClick={() => adquirirItem(produto.preco)}
                className="bg-blue-500 text-white py-2 px-4 rounded-full hover:bg-blue-600 transition mt-auto"
              >
                ADQUIRIR
              </button>
            </div>
